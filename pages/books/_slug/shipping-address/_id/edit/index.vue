<template>
  <div>
    <NavBar></NavBar>
    <div class="heading">Edit shipping address</div>
    <div class="form">
      <div class="address">
        <baseInput
      ...
      <div class="default">
        <input id="checkbox" v-model="address.default" type="checkbox" />
        <label v-if="address.default" for="checkbox" class="label"
          >Remove as default address</label
        >
        <label v-else for="checkbox" class="label"
      ...
      <div class="mt-1 mb-1">
        <Button
          ...
          @click="submit"
          ><div v-if="!spinner">edit address</div>
...
export default {
  ...
  methods: {
    async submit() {
      ...
      formData.append('default', this.address.default)
      formData.append('address', this.address.address)
      formData.append('region', this.address.region)
      formData.append('town', this.address.town)
      formData.append('phone', this.address.phone)

      try {
        this.spinner = true
        await axios.put(
          `${process.env.baseUrl}/user-address/${this.$route.params.id}/`,
          formData,
          {
            headers: {
